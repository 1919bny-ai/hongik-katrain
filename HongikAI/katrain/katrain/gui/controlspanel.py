...
class PlayAnalyzeSelect(MDFloatLayout):
    ...
    def save_ui_state(self):
        # 'uistate' 섹션이 있는지 확인하고 없으면 새로 추가합니다.
        if not self.katrain.config.has_section('uistate'):
            self.katrain.config.add_section('uistate')

        # 이제 'uistate' 섹션이 보장되므로 안전하게 값을 설정할 수 있습니다.
        # Kivy의 config 객체는 내부적으로 딕셔너리처럼 동작하므로 아래 코드는 유효합니다.
        ...
        self.katrain.save_config("uistate")
...
class ControlsPanel(BoxLayout):
    ...
    def update_players(self, *_args):
        for bw, player_info in self.katrain.players_info.items():
            ...
            if player_info.player_type == PLAYER_HUMAN:
                ...
                rank_val = player_info.calculated_rank
                # 등급이 "학습 중" 같은 문자열인지, 숫자인지 확인
                if isinstance(rank_val, str):
                    self.players[bw].rank = rank_val  # 문자열이면 그대로 사용
                else:
                    self.players[bw].rank = rank_label(rank_val) # 숫자이면 서식 적용
    ...
    def update_evaluation(self, *_args):
        ...
        last_player_was_ai_playing_human = (katrain.last_player_info.player_type == PLAYER_AI) and (katrain.next_player_info.player_type == PLAYER_HUMAN)
        both_players_are_robots = (katrain.last_player_info.player_type == PLAYER_AI) and (katrain.next_player_info.player_type == PLAYER_AI)
        ...
        if self.active_comment_node.analysis_exists:
            ...
            points_lost_value = self.active_comment_node.points_lost
            self.stats.points_lost = float(points_lost_value) if points_lost_value is not None else None
    ...
    def update_timer(self, _dt):
        ...
        if current_node:
            ...
            byo_len = max(1, self.katrain.config("timer/byo_length") or 30)
            byo_num = max(1, self.katrain.config("timer/byo_periods") or 3)
            sounds_on = self.katrain.config("timer/sound")
            player = self.katrain.next_player_info
            ai = (player.player_type == PLAYER_AI)
            used_period = False
