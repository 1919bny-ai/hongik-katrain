...
class BadukPanWidget(Widget):
    ...
    game_over_message = StringProperty("Game over")
    ...
    def on_touch_up(self, touch):
        ...
        if self.selecting_region_of_interest:
            ...
            if (
                ...
                and self.katrain.next_player_info.player_type != PLAYER_AI
    ...
    def eval_color(self, points_lost, show_dots_for_class: List[bool] = None) -> Optional[List[float]]:
        thresholds_from_config = self.trainer_config.get("eval_thresholds", [])
                
        if isinstance(thresholds_from_config, str): 
            try:
                 eval_thresholds = eval(thresholds_from_config)
                 if not isinstance(eval_thresholds, list): 
                     eval_thresholds = [-1.0, 0.5, 1.5, 3.0, 5.0, 7.5, 10.0]
            except: 
                 eval_thresholds = [-1.0, 0.5, 1.5, 3.0, 5.0, 7.5, 10.0]
        else: 
             eval_thresholds = thresholds_from_config
    ...
    def draw_board_contents(self, *_args):
        ...
        # badukpan.py 파일 내부
        MOVE_NUMBER_COLORS = {
            'B': (0.1, 0.1, 0.1, 0.95),  # 흰 돌 위에 표시될 글자 색 (진한 회색)
            'W': (0.9, 0.9, 0.9, 0.95)   # 검은 돌 위에 표시될 글자 색 (밝은 회색)
        }
        ...
        with self.canvas:
            ...
            show_dots_for = {
                p: self.trainer_config.get("eval_show_ai", True) or katrain.players_info[p].human for p in Move.PLAYERS
            }
            show_dots_for_class = self.trainer_config.get("show_dots", [True] * 6)
            ...
            for i, node in enumerate(nodes[::-1]):  # reverse order!
                ...
                for m in node.moves + placements:
                    ...
                    if has_stone.get(m.coords) and not drawn_stone.get(m.coords):
            ...
            # 'n'키 스위치가 켜져 있을 때 숫자 그리기
            if katrain.show_move_numbers:
                nodes_with_moves = [node for node in katrain.game.current_node.nodes_from_root if node.move and not node.move.is_pass]
                for i, node in enumerate(nodes_with_moves):
                    move = node.move
                    coords = move.coords
                    
                    # --- [ 핵심 수정 부분 2: 전용 색상 사용 ] ---
                    text_color = MOVE_NUMBER_COLORS[move.opponent]
                    # --- [ 수정 끝 ] ---

                    Color(*text_color)
                    draw_text(
                        pos=(self.gridpos[coords[1]][coords[0]][0], self.gridpos[coords[1]][coords[0]][1]),
                        text=str(i + 1),
            ...
            if katrain.analysis_controls.policy.active and current_node.policy:
                policy_grid = var_to_grid(current_node.policy, (board_size_x, board_size_y))
                best_move_policy = max(*current_node.policy)
                colors = Theme.EVAL_COLORS[self.trainer_config["theme"]]
                text_lb = 0.01 * 0.01
                for y in range(board_size_y - 1, -1, -1):
                    for x in range(board_size_x):
                ...
                with pass_btn.canvas.after:
                    move_policy = current_node.policy[-1]
    ...
    def draw_hover_contents(self, *_args):
        ...
        with self.canvas.after:
            ...
            # [최종 수정] pass circle -> game over message 로직
            # self.game_is_over가 True이거나 현재 패스 차례일 때 중앙 메시지를 표시합니다.
            if self.game_is_over or current_node.is_pass: 
                if self.game_is_over: # 게임이 종료된 상태일 경우
                    text = self.game_over_message # 우리가 만든 변수의 내용을 text로 사용
